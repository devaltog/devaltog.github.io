...
async function loadAll(){
  ...
  // Alphanumeric sorting rule: 0-9 first, then A-Z
...
function startSet(lid,type,setNo){const qs=shuffle(QUESTIONS.filter(q=>q.lessonId==lid&&q.type==type&&(q.setNo||1)==setNo));
 QUIZ={lid,type,setNo,qs,idx:0,results:new Array(qs.length).fill(null)};renderQuiz()}
...
function gradeCurrent(q){if(q.type=="MCQ"){const c=document.querySelector('[name=qa]:checked');const ok=c&&c.value==q.ans?1:0;return{ok,n:1,line:"Correct answer: "+q.ans,your:c?c.value:"(no answer)"}}
 if(q.type=="Written"){const a=(document.getElementById("qaw").value||"").toLowerCase(),k=(q.ans||"").toLowerCase().split(",").map(x=>x.trim()).filter(Boolean);const ok=k.length&&k.filter(x=>a.includes(x)).length>=Math.ceil(k.length/2)?1:0;return{ok,n:1,line:"Key points: "+q.ans,your:document.getElementById("qaw").value||"(no answer)"}}
 let ok=0;const n=(q.opts||[]).length;const your=[];(q.opts||[]).forEach((p,j)=>{const sel=(document.getElementById("qam_"+j)||{}).value||"";your.push(p.split("|")[0]+" → "+(sel||"—"));if(sel==p.split("|")[1])ok++});
 return{ok,n,line:q.opts.map(p=>p.replace("|"," = ")).join(", "),your:your.join(", ")}}
function quizNext(){const q=QUIZ.qs[QUIZ.idx];QUIZ.results[QUIZ.idx]=gradeCurrent(q);QUIZ.idx++;renderQuiz()}
function renderQuiz(){if(!QUIZ)return;
 if(QUIZ.idx>=QUIZ.qs.length){let sc=0,tot=0,fb="";QUIZ.results.forEach((r,i)=>{sc+=r.ok;tot+=r.n;fb+=`<div class="item ${r.ok==r.n?"good":"bad"}"><span>Q${i+1}: ${r.ok}/${r.n} — <small>Your answer: ${esc(r.your)}</small></span><small>${esc(r.line)}</small></div>`});
